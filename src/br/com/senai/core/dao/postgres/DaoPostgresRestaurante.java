...
public class DaoPostgresRestaurante implements DaoRestaurante {
	...
	private String SELECT_RESTAURANTES_ALL = "SELECT r.id id_restaurante, r.nome nome_restaurante, r.descricao, r.cidade, r.logradouro, r.bairro, r.complemento, c.id id_categoria, c.nome nome_categoria FROM restaurantes r, categorias c WHERE r.id_categoria = c.id order by r.nome";
	...
	public ArrayList<Restaurante> listarTodos() {
		ArrayList<Restaurante> listarRestaurantes = new ArrayList<Restaurante>();
		
		PreparedStatement ps = null;
		ResultSet rs = null;
		
		try {
			ps = conexao.prepareStatement(SELECT_RESTAURANTES_ALL);
			rs = ps.executeQuery();
			
			while (rs.next()) {
				
				listarRestaurantes.add(extrairDo(rs));
			}
			
		} catch (Exception e) {
			throw new RuntimeException("Ocorreu um erro ao listar os restaurantes. Motivo: " + e.getMessage());
		} finally {
			ManagerDb.getInstance().fechar(ps);
			ManagerDb.getInstance().fechar(rs);
